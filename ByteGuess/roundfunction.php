...
class GameRound{
	...
	function insGameDetails($conn,$id,$gameID){
		...
		// $this->getTeamPlayed($conn,$gameID);
        ...
        $return = $this->cardRiddlesGroup($conn,$gameID);
		...
		// $return['ins_round']		= $this->roundDetails($conn,$gameID);
    ...
    function topnav(){
		...
		$return= '<div class="top-nav">
                    ...
                    <div class="nav-left">
                        <div class="logo">
                            <img src="../assets/images/ERM sandbox.png" alt="SARAS Simulations" height="50" title="SARAS Simulations" />
                        </div>
                    </div>
                    ...
                    <div class="nav-right">
                        <button class="nav-btn" onclick="goToGame()">Go to Game Page</button>
    ...
    // print_r($insGameDetails['cardgroup']['cg_guidelines']); die;
		...
		// if(count($insGameDetails['cardgroup']) > 0){			
		...
		// 	$return.= $this->casestudy($insGameDetails['cardgroup'],'inc','',$next);
        //     // print_r($return); die;
	...
	function findNext($insGameDetails,$type,$round=0,$rID=0,$fround=0){	
		...
		elseif($type==2){
			...
			$caseStudy = $insGameDetails['ins_round']['round_'.$round]['casestudy'] ?? [];
			$treasure  = $insGameDetails['ins_round']['round_'.$round]['treasure'] ?? [];

			if (is_array($caseStudy) && count($caseStudy) > 0) {

			    $return = 'onclick="casestudy(\'round_'.$round.'\',1,1,0,0,0,'.$baType.',0);"';

			}
			else if (is_array($treasure) && count($treasure) > 0) {
		...
		elseif($type==3){
			...
			if(is_array($treasure) && count($treasure) > 0){
		...
		elseif($type==4){
			...
			$roundKey  = 'round_'.$round;
			$nRoundKey = 'round_'.$nround;

			/* Safe array extraction */
			$stageEnd    = $insGameDetails['ins_round'][$roundKey]['stage_end'] ?? [];
			$nextStage   = $insGameDetails['ins_round'][$nRoundKey]['stage_start'] ?? [];
			$caseStudy1  = $insGameDetails['ins_round']['round_1']['casestudy'] ?? [];
			$treasure1   = $insGameDetails['ins_round']['round_1']['treasure'] ?? [];

			if (is_array($stageEnd) && count($stageEnd) > 0) {
	...
	function findPre($insGameDetails,$type,$round='',$rID=0){
		...
		if($type==2){
			...
			if($round==1){
				...
				$pRoundKey = 'round_'.$pRound;

/* Safe extraction (null → empty array) */
$stageEnd = $insGameDetails['ins_round'][$pRoundKey]['stage_end'] ?? [];
$treasure = $insGameDetails['ins_round'][$pRoundKey]['treasure'] ?? [];
...
elseif (is_array($treasure) && count($treasure) > 0) {
...
$stageStart = $insGameDetails['ins_round'][$roundKey]['stage_start'] ?? [];
$stageEnd   = $insGameDetails['ins_round'][$pRoundKey]['stage_end'] ?? [];
$treasure   = $insGameDetails['ins_round'][$pRoundKey]['treasure'] ?? [];
$incCase    = $insGameDetails['ins_casestudy'] ?? [];

if (is_array($stageStart) && count($stageStart) > 0) {
...
elseif ($round == 1) {
    ...
    if (is_array($incCase) && count($incCase) > 0) {
...
elseif (is_array($stageEnd) && count($stageEnd) > 0) {
...
/* Safe extraction (null → empty array) */
$caseStudy   = $insGameDetails['ins_round'][$roundKey]['casestudy'] ?? [];
$stageStart  = $insGameDetails['ins_round'][$roundKey]['stage_start'] ?? [];
$incCase     = $insGameDetails['ins_casestudy'] ?? [];
$stageEnd    = $insGameDetails['ins_round'][$pRoundKey]['stage_end'] ?? [];
$treasure    = $insGameDetails['ins_round'][$pRoundKey]['treasure'] ?? [];
...
if (is_array($caseStudy) && count($caseStudy) > 0) {
    ...
    $count  = count($caseStudy);
    $return = 'onclick="casestudy(\'round_'.$round.'\','.$count.',1,0,0,0,0,0);"';

}
elseif (is_array($stageStart) && count($stageStart) > 0) {
...
elseif (is_array($treasure) && count($treasure) > 0) {
	...
	function stageMsg($msgVal,$div,$pr='',$nx=''){
		...
		// if ($nHand == 'cblock') {
		//     header("Location: ./teaminstance_be.php"); 
		//     // exit(); 
		// }
...
//End code By Maria	
	...
	function casestudy($casestudy,$div,$pr='',$nx=''){
		...
		// }
		  foreach($casestudy['cg_guidelines'] as $casVal){
			if($pr==''){
				$class 	= ($si==1)?'disblk':'disnone';
				$backBtn = ($si==1) ? '<div class="cas_empty"></div>' : '<div class="arrow-btn left chand" title="Previous" onclick="casestudy(\''.$div.'\','.($si-1).');">Previous</div>';
			}
			else{
				$class 	= 'disnone';
				$backBtn = '<div class="arrow-btn left" title="Previous" '.$pr.'>Previous</div>';
			}
			
			// --- Next Arrow (Right) ---
			$nextAction	= ($insCaseCount==$si)? $nx : 'onclick="casestudy(\''.$div.'\','.($si+1).');"';
			$nHand = ($insCaseCount==$si && $nx == '') ? 'cblock' : 'chand';
			$nextBtn = '<div class="arrow-btn right" title="Next" '.$nextAction.'>Next</div>';
			
			// Process the case study content
			$richContent = '';
			if(!empty($casVal['cg_guidelines'] ?? '')){
				$clean_string = $casVal['cg_guidelines'] ?? '';
				$parts = preg_split('/(?=\d+\.)/', $clean_string, -1, PREG_SPLIT_NO_EMPTY);
				
				foreach ($parts as $item) {
					if (preg_match('/(\d+\.)\s*\*\*(.*?)\*\*(?:\s*:\s*)?(.*)/s', $item, $matches)) {
						$number      = $matches[1];
						$label       = $matches[2];
						$description = isset($matches[3]) ? $matches[3] : '';
						$richContent .= "<h4>$number $label</h4><p>$description</p>";
					}
				}
			}
			...
			$return.='<div class="allcase '.$class.' cspos" id="'.$div.'_'.$si.'">
						<div class="guideline-visual">
							<img src="../assets/images/company.jpeg" alt="Guideline Image">
						</div>
