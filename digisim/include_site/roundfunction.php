...
class GameRound{
	...
	function getTeamPlayed($conn,$accID){
		...
		$this->data = !empty($row['data']) ? json_decode($row['data'], true) : [];

		//$this->apiData 		= ($row['api_data']!='')?json_decode($row['api_data'],true):array();
		$this->apiData = !empty($row['api_data']) ? json_decode($row['api_data'], true) : [];
	...
	//=== Body Load
	function bodyLoad($insGameDetails, $conn, $rid, $accID,  $instanceID){
		$return = '';
	 	$sql 	= "SELECT
							...
							WHERE (gm_instance_round.ir_instance_pkid ='".$instanceID."'
			...
			$max 	= $row['maxlimit'];
			// Get used (opened) cards
			$sql = "SELECT data FROM team_play_details WHERE team_play_pkid = '".$accID."'";
			$result = $conn->query($sql);
			...
			if($result && $result->num_rows > 0){
				...
				if(is_array($data)){
					$used = $data['round'][$rid]['cardused'] ?? 0;
				}
			}
			...
			$return.= '<div class="scorediv disnone" id="scorebox">
					...
					<div class="maxdiv disnone " id="maxbox">
						...
						<div class="stat-block">
							<div class="maxcards stat-box "> 
							<i class="fa-regular fa-eye"></i> Cards Revealed </div>
							<div class="maxcount" id="maxcard">'.$used.'/'.$max.'</div>
						...
						<div class="stat-block">
							<div class="balancecards stat-box " id="balancecard"> 
		...
		foreach($treasure as $treasureName){
			...
			$box.='<div class="topindexdiv" >
						...
						<div class="checkhuntgame sec3Button55 handpoint" id="'.$hid.'-'.$div.'-'.$rID.'"  style="background-image:url(learning/'.$treasureName['lu_image'].');" title="'.$treasureName['lu_name'].'" >							
							...
							<div class="tabsubBoxText indexcat" style="height: 22%;">'.$treasureName['lu_name'].'sss</div>
			...
			if($tCount!=$ti && (!isset($this->data['code']['treasure-'.$hid]))){
				...
				$checkBut='<div class="checkDiv">'.$this->msgAI['txt'][8].'</div>
								...
								<div class="checkDiv"><button type="button" class="submitbut checkbutton" id="'.$hid.'-'.$rID.'" style="margin-top: auto;">CHECK</button></div>';
	...
	function cardAnswer($rID,$div,$treasure,$getClues){
		...
		if(isset($this->data['answer']['round_'.$rID])){
			...
			$submitBut = '<div class="checkDiv padl35"><button type="button" class="submitbut checkanswer finalbtn" id="'.$rID.'" style="margin-left: 40%;margin-top: auto;">SUBMIT YOUR DECISION.</button></div>';
		...
		$return = '<div class="allcase disnone" id="cardbox_'.$rID.'">
						...
						<div class="casestudy_bt">
							...
							<div class="casestudy_hunt finalsubmit">
								...
								<div class="cas_left chand" title="Next" onclick="huntView('.$rID.',\''.$div.'_1\',2,0);"></div>
	...
	//=== DigiSim Main
// By Maria Message Center left pannel
 function digiSimDiv($treasure,$div,$pr='',$nx='',$rID=0,$roundID=0){
	 $simID = $treasure['simid'];
		...
		foreach($treasure['msg'] as $msgVal){
			...
			$msgTd .= '<div class="nav-item '.$activeClass.'" data-ch="'.$msgVal['ch_id'].'">
							<img src="images/channels/'.$msgVal['ch_image'].'" width="24" height="24" alt="'.$msgVal['ch_level'].'" title="'.$msgVal['ch_level'].'"> '.$msgVal['ch_level'].'
							<span class="badge blue">
								<!-- Trigger 1 unread -->
								<!--<div id="unread_start_'.$msgVal['ch_id'].'"></div>-->

								<!-- Trigger 2 & 3 unread (existing logic) -->
								<div id="view_'.$roundID.'_'.$msgVal['ch_id'].'"></div>
							</span>
						</div>';
			$isFirst = false;
		}	
	...
	$msgTable = '';
		...
		$msgTable .='<table width="100%" border="0" cellspacing="0" cellpadding="0" class="padl20 padt20">
					  <tr>
						<td align="left" colspan="2" valign="top" class="simtitle2"><span class="simtitle">'.$msgT.'</span><p>'.$msgM.'</p></td>
					  </tr></table>';

		$msgTable .= '<div class="app-container">

				<!-- ================= SIDEBAR ================= -->
				<aside class="sidebar" id="sidebar">
					<div class="nav-top">
						<span class="nav-label">STANDARD</span>
						'.$msgTd.'
					</div>
				...
				$msgTable .= '
						<section class="msg-list">
							...
							<!-- ONLY messages go here -->
							<div id="msgItems">	</div> <!-- #msgItems -->
		</section>';

		$msgTable .= '
			<main class="content-view" id="contentView">
				<button class="back-btn" onclick="closeMessage()">
				...
				<div id="renderBody">
					...
					<div class="content-header">
						<div class="agent-info">
							...
							<div class="tag-row">
								<h2 id="v-sender">Agent Delta</h2>
								<span class="tag-pill"></span>
							</div>
						</div>
					</div>
					<article>
						<p id="v-body" class="body"></p>
					</article>
				</div>
			</main>
		</div>';
	$msgTable .= '
		...
		// ================= START TRIGGER-1 UNREAD =================

		function updateStartUnreadCount(chID) {
			const count = currentMessages.filter(
				m => m.dm_trigger == 1 && !m.is_read
			).length;

			const el = document.getElementById("unread_start_" + chID);
			if (!el) return;
		...
		function renderMessages() {
			const msgBox = document.getElementById("msgItems");
			msgBox.innerHTML = "";

			let list = currentMessages;

			if (activeTab === "unread") {
				list = currentMessages.filter(m => !m.is_read);
			}

			if (list.length === 0) {
				msgBox.innerHTML = "<div class=\'no-msg\'>No messages</div>";
				return;
			}
			...
			list.forEach(m => {
				...
				div.innerHTML = "<div class=\'card-subject\'>" + escapeHtml(m.subject) + "</div>";
				div.onclick = () => openMessage(div, m);
		...
		function openMessage(el, msg) {

			document.getElementById("v-sender").innerText = msg.subject;
			document.getElementById("v-body").innerHTML = msg.body;

			if (!msg.is_read) {

				fetch("include_site/mark_read.php", {
					method: "POST",
					headers: { "Content-Type": "application/x-www-form-urlencoded" },
					body: "dm_id=" + msg.dm_id
				});

				msg.is_read = true;
				updateStartUnreadCount(currentChannel);
			}

			activeTab = "all";
			document.querySelector(".list-tabs div:nth-child(1)").classList.add("tab-active");
			document.querySelector(".list-tabs div:nth-child(2)").classList.remove("tab-active");
		...
		function loadMessages(chID) {
			...
			fetch(
				"include_site/load_messages.php?sim_id=" + GAME_ID +
				"&channel_id=" + chID +
				"&trigger=1"
			)
			.then(r => r.json())
			.then(data => {
				...
				updateStartUnreadCount(chID);

				activeTab = "unread";
				document.querySelector(".list-tabs div:nth-child(2)").classList.add("tab-active");
				document.querySelector(".list-tabs div:nth-child(1)").classList.remove("tab-active");
		...
		document.addEventListener("DOMContentLoaded", function () {
			...
			document.querySelectorAll(".list-tabs div").forEach(tab => {
				tab.addEventListener("click", function () {
					document.querySelectorAll(".list-tabs div")
						.forEach(t => t.classList.remove("tab-active"));

					this.classList.add("tab-active");
					activeTab = this.innerText.toLowerCase().includes("unread") ? "unread" : "all";
					renderMessages();
				});
			});
			...
			document.querySelectorAll(".nav-item").forEach(item => {
				item.addEventListener("click", function (e) {
					...
					loadMessages(this.dataset.ch);
				});
			});

			const firstChannel = document.querySelector(".nav-item.active");
			if (firstChannel) loadMessages(firstChannel.dataset.ch);
		});
		...
		foreach($treasure['score'] as $scoreVal){
			...
			$pri[$scoreVal['stv_id']] = '<div class="survivalclue dgiclmgnl" '.$colore.' '.$drag.' id="drg'.$roundID.'_'.$scoreVal['stv_id'].'" >'.$scoreVal['stv_sname'].'</div>
										<div class="scorenameview" style="color:'.$scoreVal['stv_color'].';">'.$scoreVal['stv_name'].'</div>
		...
		$submitBut = '';
		if(!isset($this->data['bcm'][$roundID]['submit'])){
			$submitBut = '<div class="checkDiv padl20 setdesc1">
									<div class="setdesc2">
										'.$scoreVal['st_desc'].'
									</div>
							</div>
							
							<div class="checkDiv padl35"><button type="button" class="submitbut savebcm" id="'.$roundID.'" >SAVE</button></div>
		...
		/*<div class="boxmsgpos"  id="bell_div_'.$roundID.'">
				<div class="notification-boxmsg">
					<span id="bell_text_'.$roundID.'"></span>
		...
		$return .='<div class="allcase disnone" id="'.$div.'_2">
					<div class="rightsidebar">
						<div class="mgnt70">&nbsp;</div>
						<div class="surlist" id="priority_L_'.$roundID.'_2" >
	...
	function viewBcmMsg($conn,$accID,$ins,$rid,$channID){		
		...
		$full	= array();
		$getMsh = array();
		$sql 	= "SELECT * FROM digisim_message WHERE dm_injectes_pkid ='".$channID."' AND dm_trigger = 1";
		$result	= $conn->query($sql);
		while($row = $result->fetch_assoc()) {
			$getMsh[] = $row;
		...
		if(isset($data['bcm'][$rid]['msg'])){
			...
			if(count($msgIDs) > 0){
				$sql 	= "SELECT * FROM digisim_message WHERE dm_injectes_pkid ='".$channID."' AND dm_id IN(".implode(',',$msgIDs).")";
				$result	= $conn->query($sql);
				while($row = $result->fetch_assoc()) {
					$getMsh[] = $row;
					$data['bcm'][$rid]['viewmsg'][]	  = $row['dm_id'];
...
//=== View Survival positive negative Msg
	...
	function submitDigiBcm($conn,$proArray,$accID,$ins,$simid){
		...
		//=== Get Score Types
		$setScoreVal = array();
		$sqlS 	= "SELECT stv_id,stv_value FROM scoretype_value WHERE stv_scoretype_pkid = '".$scorepkid."'";
		...
		//==== Get response
		$serUserScore = array();
		$sql 	= "SELECT gm_instance_round.ir_order AS roundorder, digisim_response.dr_score_pkid, digisim_response.dr_id
		FROM gm_round INNER JOIN gm_instance_round ON (gm_round.ro_id = gm_instance_round.ir_round_pkid) 
		INNER JOIN digisim ON (gm_round.ro_treasure_group_pkid = digisim.di_id) INNER JOIN sub_index ON (digisim.di_response_id = sub_index.ix_group_pkid) INNER JOIN digisim_response ON (sub_index.ln_id = digisim_response.dr_response_pkid) WHERE (sub_index.ln_status ='1' AND gm_instance_round.ir_instance_pkid ='".$ins."' AND gm_instance_round.ir_round_pkid ='".$rid."')		
            ...
            if($si == 1 && $div == 'inc') {
                ...
                for($i=1; $i<=$totalRounds; $i++){
					...
					if($ri <= $cuRound){
                     ...
                     <div class="tracker-item future"><div class="tracker-circle" style="background-color:#4CB050;"><span style="font-size: 14px;margin-top: -5px;color:#FBA54E;">✔️</span></div></div>';
					}else if($neRound==$ri){
					$trackerNodes .= '<div class="tracker-line"></div>
                     <div class="tracker-item future fas"><div class="tracker-circle" style="background-color:#f6ae40;"><span style="font-size: 15px;margin-top: -5px;">🔓</span></div></div>';
					}
					else{
					 $trackerNodes .= '<div class="tracker-line"></div>
                     <div class="tracker-item future"><div class="tracker-circle"><span style="font-size: 15px;margin-top: -5px;">🔒</span></div></div>';	
					}
				$ri++;
                }    
				if($ri == $cuRound){
